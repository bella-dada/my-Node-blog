...
router.post("/add", async (req, res, next) => {
  // 获取标题和内容及分类
  let { title, content, classname01, classname02, classname03, type, pic_url } =
    req.body;
  ...
  try {
    ...
    if (!classname01) {
      ...
      (await querySql("select classify_id from classify where classname = ?", [
        classname01,
      ])) == false
    ) {
      ...
      //  再次获取分类id和分类名称
      var cid01 = await querySql(
        "select classify_id from classify where classname = ?",
        [classname01]
      );
      cid01 = cid01[0].classify_id;
      var className_01 = await querySql(
        "select classname from classify where classname = ?",
        [classname01]
      );
    ...
    if (!classname02) {
      ...
      //  再次获取分类id和分类名称
      var cid02 = await querySql(
        "select classify_id from classify where classname = ?",
        [classname02]
      );
      cid02 = cid02[0].classify_id;
      var className_02 = await querySql(
        "select classname from classify where classname = ?",
        [classname02]
      );
    ...
    if (!classname03) {
      ...
      //  再次获取分类id和分类名称
      var cid03 = await querySql(
        "select classify_id from classify where classname = ?",
        [classname03]
      );
      cid03 = cid03[0].classify_id;
      var className_03 = await querySql(
        "select classname from classify where classname = ?",
        [classname03]
      );
...
router.get("/detail", async (req, res, next) => {
  ...
  try {
    ...
    // 将文章访问量＋1后更新到文章中
    await querySql("update article set visited = ? where id = ?", [
      visited,
      article_id,
    ]);
...
//     }
//     var coust = await querySql(sql, [keyWords]);
//     res.send({ code: 0, msg: "搜索成功", data: coust });
//   } catch (error) {
//     console.log(error);
//     next(error);
//   }
...
router.post("/search", async (req, res, next) => {
  ...
  try {
    ...
    WHERE title LIKE '%${keyWords}%'|| content LIKE '%${keyWords}%'`;
    let result = await querySql(sql);
    let arr = result.map((item) => {
      if (item.classify !== "[]" && item.classify != null) {
    ...
    if (curPage && pageSize) {
      if (!type) {
        ...
        var sql =
          ...
          DATE_FORMAT(create_time,"%Y-%m-%d-%H:%i:%s") AS create_time FROM article where type = 0 limit ` +
          start +
          "," +
          pageSize;
      ...
      if (type == 1) {
        ...
        var sql =
          `select id,title,content,
          class_name01,class_name02,class_name03,type,pic_url,like_count,
          DATE_FORMAT(create_time,"%Y-%m-%d %H:%i:%s") AS create_time from article where type =` +
          type;
      } else {
        // 否则返回技术文章 type为0
...
router.get("/classify/single", async (req, res, next) => {
  try {
    ...
    res.send({
      code: 0,
      msg: "获取单篇博客分类成功",
      data: { classname01, classname02, classname03 },
    });
  } catch (e) {
    console.log(e);
    next(e);
...
router.post("/update", async (req, res, next) => {
  ...
  try {
    ...
    // 通过 类id 修改类名表里的数据
    if (
      (await querySql("select classname from classify where classname = ?", [
        classname01,
      ])) == false
    ) {
