...
import ArticleLists from "../articleLists/ArticleLists.vue";
export default {
  data() {
    return {
      inputVal: null,
  ...
  methods: {
      ...
      if (this.inputVal) {
        this.$http
          ...
          .then((res) => {
            ...
            this.$store.state.searchArticleList = this.searchArticle;
    ...
    newSearchList() {
      this.$store.commit("newSearchList", this.searchArticle);
    },
    setInputKeyWords() {
      this.$store.commit("setInputKeyWords", this.inputVal);
    },
    onFocus() {
      console.log("获取焦点");
      this.show = !this.show;
